...
void SlotMachineWindow::initializeReels() {
    ...
    for (int row = 0; row < REEL_ROWS; ++row) {
        for (int col = 0; col < REEL_COLS; ++col) {
            ...
            // configure sprite properties (texture, origin, scale, position)
    ...
    // create vertical grid lines for the grid where the symbols are placed
    ...
    // create horizontal grid lines for the grid where the symbols are placed
    ...
    //set up and style the background panel for reels
...
void SlotMachineWindow::updateHighlights() {
    // if there are no winning paylines, reset the sprites to white
    if (!isAnimating) {
    ...
    // calculate glow effect using sine wave for smooth animation
    float time = highlightClock.getElapsedTime().asSeconds();
    highlightIntensity = (std::sin(time * 5) + 1.0f) / 2.0f;


    // apply yellow glow to winning symbols, keep the others white
...
void SlotMachineWindow::highlightWinningSymbols(const std::vector<std::vector<Symbol>>& results, double payout) {
    // seset all the highlights
    std::fill(highlightedSprites.begin(), highlightedSprites.end(), false);


    // check for winning combinations if there was a payout
    if (payout > 0) {
        for (const auto& payline : gameLogic.getPaylines()) {
            ...
            // first symbol in current payline, because winning combination start from the left side
            ...
            // if enough matches found, mark those positions for highlighting
...
void SlotMachineWindow::handleSpin() {
    ...
    if (currentCredits >= currentBet) {
        ...
        // show win message and play sound if won
        if (payout > 0) {
            ...
            // if there are no winning then empty message
            winningsText.setString("");
        }
    }
    // show error if not enough credits
    else {
...
void SlotMachineWindow::run() {
    while (window.isOpen()) {
        ...
        // format and update credit/bet displays
